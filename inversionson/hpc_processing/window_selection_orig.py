...
def select_windows(
    ...
    #amped_windows = []
    #for j in flatnotmasked_contiguous(final_windows):
        # Again assert a certain minimal length.
    #    if (j.stop - j.start) < min_length:
    #        continue

        #cpy = tr.copy()
        #cpy.trim(starttime = window[0], endtime = window[1])
    #    if np.max(data[j.start : j.stop])/np.max(data) < max_amplitude_window_ratio:
    #            continue

    #    amped_windows.append((j.start, j.stop))
